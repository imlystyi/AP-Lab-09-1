...
int WellDoingStudentsCount(Student* students, const int studentsCount)
{
    int wellDoingStudentsCount = 0;

    for (int ii = 0; ii < studentsCount; ii++)
        if (students[ii].physicsGrade == 5 && students[ii].mathGrade == 5 && students[ii].programmingGrade == 5) wellDoingStudentsCount++;

    return wellDoingStudentsCount;
}

float WellDoingPhysicsStudentsPercent(Student* students, const int studentsCount)
{
    int wellDoingPhysicsStudentsCount = 0;

    for (int ii = 0; ii < studentsCount; ii++)
        if (students[ii].physicsGrade == 5) wellDoingPhysicsStudentsCount++;
...
int main()
{
    ...
    cout << "\nКількість студентів, які вчаться на \"відмінно\": " << WellDoingStudentsCount(students, studentsCount)
        << "\nВідсоток студентів, які отримали з фізики оцінку \"5\": " << WellDoingPhysicsStudentsPercent(students, studentsCount) << "%" << endl;
}
